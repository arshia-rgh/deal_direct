...
def mock_send_request():
    with patch("apps.accounts.views.payment_views.send_request") as mock:
        yield mock


@pytest.fixture
def mock_verify():
    with patch("apps.accounts.views.payment_views.verify") as mock:
        yield mock


@pytest.fixture
def mock_update_wallet_balance():
    with patch("apps.accounts.views.payment_views.update_wallet_balance") as mock:
        yield mock
